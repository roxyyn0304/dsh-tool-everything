...
async function runQuery(exec, input, caps) {
	...
	for (let index = 0; index < count; index++) {
		...
		const sizeOk = api.GetResultSize(index, sizeBuf);
		const size = sizeOk ? Number(koffi.decode(sizeBuf, "int64")) : -1;
...
function applyTool(ctx, caps) {
	...
	const tool = defineTool({
		...
		async execute(args, exec) {
			...
			return await runQuery(exec, input, { ...caps, maxResults });
		},
		presentCall,
		presentResult
	});
...
const Config = z.object({
	...
	everythingDllPath: z.string().optional()
});
